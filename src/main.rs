...
fn start_client(dest: String, config_path: std::path::PathBuf, file_name: std::path::PathBuf) {
    let mut buffer = vec![0u8; 128 * 1024 * 1024];
    let mut stream = TcpStream::connect(dest).unwrap();
    let (reader, writer) = &mut (&stream, &stream);
    let mut open_option = OpenOptions::new();
    match open_option.read(true).open(config_path.join(&file_name)) {
        Ok(mut file) => {
            let file_info = FileInfo::new(file.metadata().unwrap().len(), String::from(file_name.to_str().unwrap()));
